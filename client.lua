...
local UITheme = require 'modules.ui.client'
local ClientPerformance = require 'modules.performance.client'
local PropManager = require 'modules.props.client'
local currentWeapon
...
function client.openInventory(inv, data)
	local usebackpack = checkBackpackInslot()
	if invOpen then
    ...
    if inv == 'player' or inv == 'showbpk' and data ~= cache.serverId then
		...
		if inv == 'showbpk' then
			local check = true
			if GlobalState.usebackpackItem then
				check = lib.callback.await('ox_inventory:getbackpackitem', 200, data)
			end
...
-- Helper function to spawn props for a drop (MUST BE BEFORE RegisterNetEvent)
local function spawnDropProps(point, forceRefresh)
	if point.instance and currentInstance and point.instance ~= currentInstance then 
		return 
	end
	
	-- Clear existing props if refreshing
	if forceRefresh and point.entities then
		for _, entity in ipairs(point.entities) do
			if DoesEntityExist(entity) then
				if PropManager then
					PropManager.UnregisterProp(entity, point.invId)
				end
	...
	-- Only spawn if not already spawned (unless force refresh)
	if point.entities and not forceRefresh then 
		return 
	end
	
	
	-- Use lib.callback.await with correct parameter order (callback name, timeout/false, ...args)
	local dropInventory = lib.callback.await('ox_inventory:getDropItems', false, point.invId)
	
	if not dropInventory then 
		return 
	end
	
	if not dropInventory.items then
		return
	end
	...
	for slot, item in pairs(dropInventory.items) do
		if item and item.name then
			...
			print(('[DEBUG] Empty slot: %d'):format(slot))
		end
	end
	...
	-- Check if can spawn props (performance)
	if ClientPerformance and not ClientPerformance.CanSpawnProp(itemCount) then
		return
	end
	
	local baseCoords = point.coords
	local index = 0
	
	-- Spawn a prop for each item in the drop
	for slot, item in pairs(dropInventory.items) do
		if item and item.name then
			local startTime = GetGameTimer()
			
			-- Determine model for this specific item
			local itemData = Items(item.name)
			local model = client.dropmodel
			
			if config and config.DropSystem and config.DropSystem.enableCustomProps and itemData then
				if itemData.dropmodel then
					model = itemData.dropmodel
				elseif itemData.ammoname or itemData.durability then
					model = config.DropSystem.categoryModels.weapon or model
				elseif item.name:find('ammo') then
					model = config.DropSystem.categoryModels.ammo or model
				elseif itemData.client and itemData.client.status then
					if itemData.client.status.hunger then
						model = config.DropSystem.categoryModels.food or model
					elseif itemData.client.status.thirst then
						model = config.DropSystem.categoryModels.drink or model
					end
				elseif item.name:find('money') then
					model = config.DropSystem.categoryModels.money or model
				end
			end
			
			-- Use point.model if provided (from server for backward compatibility)
			if point.model and index == 0 then
				model = point.model
			end
			...
			-- Calculate offset position for multiple items (spiral pattern)
			local offsetX, offsetY = 0, 0
			if index > 0 then
				-- Increased spacing: 0.5m base radius, 0.5m per ring, 6 items per ring
				local itemsPerRing = 6
				local baseRadius = 0.5
				local radiusIncrement = 0.5
				
				local ring = math.floor((index - 1) / itemsPerRing)
				local positionInRing = (index - 1) % itemsPerRing
				local radius = baseRadius + (ring * radiusIncrement)
				local angle = (positionInRing / itemsPerRing) * (math.pi * 2)
			...
			if ClientPerformance then
				ClientPerformance.TrackPropRendered(loadTime)
			end
			
			-- Register prop for memory leak prevention
			if PropManager then
				PropManager.RegisterProp(entity, point.invId, model)
			end
...
RegisterNetEvent('ox_inventory:updateSlots', function(items, weights)
	if source ~= '' and next(items) then 
		...
		for i = 1, #items do
			...
			if v.inventory and type(v.inventory) == 'string' and v.inventory:find('drop%-') then
				local dropId = v.inventory
				local point = client.drops and client.drops[dropId]
				if point then
					SetTimeout(500, function()
						spawnDropProps(point, true)
					end)
					break
				else
					print(('[DEBUG] Point not found in client.drops table for dropId: %s'):format(dropId))
				end
			end
		end
	end
...
local function onExitDrop(point)
	if point.entities then
		for _, entity in ipairs(point.entities) do
			if entity then
				-- Unregister prop before deletion
				if PropManager then
					PropManager.UnregisterProp(entity, point.invId)
				end
...
RegisterNetEvent('ox_inventory:removeDrop', function(dropId)
	if client.drops then
		...
		if point then
			...
			-- Handle both single entity (old) and multiple entities (new)
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	if UITheme and UITheme.LoadUITheme then
		UITheme.LoadUITheme()
	end
...
RegisterNetEvent('ox_inventory:viewInventory', function(left, right)
	...
	if client.screenblur then TriggerScreenblurFadeIn(0) end
	local usingbackpack = checkBackpackInslot()
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	if data.toType == 'player' then
		...
		elseif data.toSlot == 2 then
			CheckArmorItem(data.item)
		elseif data.toSlot == 3 then
			
		elseif data.toSlot == 4 then
			CheckParachuteItem(data.item)
		end
		...
		elseif data.fromSlot == 2 then
			CheckArmorItem(data.item, true, data.toSlot)
		elseif data.fromSlot == 3 then
		elseif data.fromSlot == 4 then
			CheckParachuteItem(data.item, true)
		end
	end
end)
