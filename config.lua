...
config.CoreFolder = 'qbx_core' -- Fill it with your Core Folder (es_extended, qb-core or qbx_core)
config.autousearmor = true          -- Automatically uses armor when player's armor drops below 5%
config.autouseparachute = true      -- Automatically deploys parachute when equipped in parachute slot

-- Item Restrictions & Classifications
config.blacklistitem = {'backpack', 'water'}        -- Items banned from backpack storage
config.ArmorItem = {'armour'}                       -- Items classified as protective armor
config.CustomMetadataItem = {'water'}               -- Items supporting custom metadata fields
...
config.backpack = {
    ...
    slot = 50,                      -- Default slot count when no backpack is equipped
    maxWeight = 500000,             -- Maximum carry weight in grams (500kg)
}

-- Special Item Slots
config.SpecialSlot = {
    {'backpack', 'water'},          -- Slot 1: Backpack (inventory expansion equipment)
    {'armour', 'water'},            -- Slot 2: Body Armor (damage protection system)
    {'phone', 'water'},             -- Slot 3: Phone (communication and applications)
    {'parachute', 'water'},         -- Slot 4: Parachute (fall damage prevention)
}
