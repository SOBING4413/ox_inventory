...
local function CustomDrop(prefix, items, coords, slots, maxWeight, instance, model)
	...
	-- Enhanced drop system with individual props support
	local itemName = nil
	local itemData = nil
	
	-- Get the first item for model determination
	if inventory.items and next(inventory.items) then
		for _, item in pairs(inventory.items) do
			if item and item.name then
				itemName = item.name
				itemData = Items(item.name)
				break
			end
		end
	end
	
	-- Determine drop model
	local dropModel = model
	if not dropModel and config and config.DropSystem and config.DropSystem.enableCustomProps and itemData then
		-- Use item-specific drop model
		if itemData.dropmodel then
			dropModel = itemData.dropmodel
		elseif itemData.ammoname or itemData.durability then
			dropModel = config.DropSystem.categoryModels.weapon
		elseif itemName and itemName:find('ammo') then
			dropModel = config.DropSystem.categoryModels.ammo
		elseif itemData.client and itemData.client.status then
			if itemData.client.status.hunger then
				dropModel = config.DropSystem.categoryModels.food
			elseif itemData.client.status.thirst then
				dropModel = config.DropSystem.categoryModels.drink
			end
		elseif itemName and itemName:find('money') then
			dropModel = config.DropSystem.categoryModels.money
		end
	end
	
	-- Fallback to default if still no model
	if not dropModel then
		dropModel = config and config.DropSystem and config.DropSystem.defaultDropModel or 'prop_box_guncase_01a'
	end
	...
	if PerformanceMonitor then
		PerformanceMonitor.TrackDropCreated(dropId)
	end
...
local function dropItem(source, playerInventory, fromData, data)
	...
	-- Enhanced drop system: Determine drop model based on item
	...
	if config and config.DropSystem and config.DropSystem.enableCustomProps and itemData then
		-- Check for custom drop model in item definition
		if itemData.dropmodel then
			dropModel = itemData.dropmodel
		-- Weapon category
		elseif itemData.ammoname or itemData.durability then
			dropModel = config.DropSystem.categoryModels.weapon
		-- Ammo category
		elseif toData.name:find('ammo') then
			dropModel = config.DropSystem.categoryModels.ammo
		...
		-- Money category
		elseif toData.name:find('money') then
			dropModel = config.DropSystem.categoryModels.money
		end
	end
	
	-- Fallback to default drop model
	if not dropModel then
		dropModel = config and config.DropSystem and config.DropSystem.defaultDropModel or 'prop_box_guncase_01a'
	end
	...
	-- Track performance
	if PerformanceMonitor then
		PerformanceMonitor.TrackDropCreated(dropId, source)
	end
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	local databackpack = {}
	if GlobalState.staticbackpack then
...
-- Get drop inventory items for multi-prop spawning
lib.callback.register('ox_inventory:getDropItems', function(source, dropId)
	local inventory = Inventory(dropId)
	
	if not inventory then 
		local found = false
		for k, v in pairs(Inventories) do
			if k == dropId or tostring(k):find(tostring(dropId)) then
				found = true
			end
		end
		if not found then
			print('[DEBUG SERVER] No matching keys found in Inventories table')
		end
		return nil 
	end
	
	
	if inventory.type ~= 'drop' then 
		return nil 
	end
