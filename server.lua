...
local PerformanceMonitor = require 'modules.performance.server'
...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	if leftBottom == false then
		if GlobalState.staticbackpack then
			exports.ox_inventory:RegisterStash('backpack-'..BackpackState.id, 'Backpack', GlobalState.backpackSettings.slot, GlobalState.backpackSettings.weight, false)
		else
			exports.ox_inventory:RegisterStash('backpack', 'Backpack', GlobalState.backpackSettings.slot, GlobalState.backpackSettings.weight, true)
		end
		Wait(150)
		leftBottom = Inventory(databackpack)
	...
	if data then
		...
		elseif isDataTable then
			...
			elseif invType == 'drop' then
				-- Enhanced drop system: Check for nearby drops and merge them
				if config and config.DropSystem and config.DropSystem.useAreaDetection then
					local dropData = Inventory.Drops[data.id]
					if dropData and dropData.coords then
						local radius = config.DropSystem.detectionRadius or 2.5
						local playerCoords = GetEntityCoords(playerPed)
						local nearbyDropIds = {data.id}
						
						-- Find all drops in the area
						for dropId, otherDropData in pairs(Inventory.Drops) do
							if dropId ~= data.id and otherDropData.coords then
								local instanceMatch = (not dropData.instance and not otherDropData.instance) or (dropData.instance == otherDropData.instance)
								if instanceMatch then
									local distance = #(dropData.coords - otherDropData.coords)
									if distance <= radius then
										table.insert(nearbyDropIds, dropId)
									end
								end
							end
						end
						
						-- If multiple drops found, use the first one but merge inventory views
						-- The actual merging happens client-side for better UX
						right = Inventory(data.id)
						if right then
							right.nearbyDrops = nearbyDropIds
						end
					else
						right = Inventory(data.id)
					end
